...
CREATE TABLE IF NOT EXISTS `s_limit` (
  ...
  `user_id` int(10) unsigned NOT NULL COMMENT '管理者id',
  `pid` int(10) unsigned DEFAULT NULL COMMENT '父级权限id',
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='后台管理系统_权限表（颗粒度到动作，即action）\r\n分为多级处理，顶层应为对应controller，子级应为对应action';
