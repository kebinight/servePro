...
CREATE TABLE IF NOT EXISTS `s_limit` (
  ...
  `status` tinyint(3) unsigned DEFAULT '1' COMMENT '是否启用：0#禁用 1#启用',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注说明',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COMMENT='后台管理系统_权限表（颗粒度到动作，即action）\r\n分为多级处理，顶层应为对应controller，子级应为对应action';
...
CREATE TABLE IF NOT EXISTS `s_menu` (
  ...
  `status` tinyint(3) unsigned NOT NULL DEFAULT '1' COMMENT '启动状态：0#禁用 1#启动',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注说明',
  `create_time` datetime DEFAULT NULL COMMENT '创建日期时间',
  `update_time` datetime DEFAULT NULL COMMENT '更改日期时间',
  PRIMARY KEY (`id`),
  KEY `pid` (`parent_id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 COMMENT='后台管理系统_菜单表';

-- 数据导出被取消选择。
-- 导出  表 servepro.s_menu_role 结构
CREATE TABLE IF NOT EXISTS `s_menu_role` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  ...
  KEY `menu_id` (`menu_id`),
  KEY `role_id` (`role_id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='后台管理系统_菜单_角色管理表（控制菜单是否对某角色可见）';
...
CREATE TABLE IF NOT EXISTS `s_role` (
  ...
  `rank` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序权重，数值越大权重越高（显示越靠前）',
  `status` tinyint(3) unsigned NOT NULL DEFAULT '1' COMMENT '状态：0#禁用 1#启用',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注说明',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更改时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=35 DEFAULT CHARSET=utf8 COMMENT='后台管理系统_角色表';
...
CREATE TABLE IF NOT EXISTS `s_role_limit` (
  ...
  `create_time` datetime NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 COMMENT='后台管理系统_角色_权限表';
...
CREATE TABLE IF NOT EXISTS `s_user` (
  ...
  `update_time` datetime DEFAULT NULL COMMENT '更改时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COMMENT='后台管理系统_用户管理表';
...
CREATE TABLE IF NOT EXISTS `s_user_role` (
  ...
  `create_time` datetime NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='后台管理系统_用户_角色';
