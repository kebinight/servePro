...
class CommonComponent extends Component
{
    ...
    const ADMIN_LOGIN_SESSION = 'Admin.login.info';
    /**
    ...
    public function failReturn($code = GlobalCode::API_OPTIONS, $errmsg = '', $msg = '请求失败', $data = [], $format = 'json') {
        $returnData = [
            ...
            'msg' => $msg,
            'data' => $data,
            'errmsg' => $errmsg ? $errmsg : GlobalCode::toString(GlobalCode::API_OPTIONS)
        ];
    ...
    public function cologinHandler($data = [], $isAppNative = false)
    {
        ...
        if (!(new \Cake\Auth\DefaultPasswordHasher)->check($data['pwd'], $user->pwd)) {
            ...
            switch ($checkRes) {
                case GlobalCode::API_NO_ACCOUNT :
                    //尚未注册完成，跳转到注册流程
                    if($user->gender == 1) {
                    ...
                    if($isAppNative) {
                        ...
                        $resData = ['status' => false,
                            'msg' => '注册未完成,请继续注册步骤',
                            'code' => GlobalCode::API_REG_NO_FIN,
                            'redirect_url' => $redirectUrl];
                        $this->anyReturn($resData);
                    } else {
                        $this->failReturn('', '', '注册未完成,请继续注册步骤', ['redirect_url' => $redirectUrl]);
                    }
                    break;
                case GlobalCode::API_FORBIDDEN_ACCOUNT :
                    $this->failReturn('forbidden account', GlobalCode::API_NO_ACCOUNT, '此账号不可用');
                    break;
            }
        ...
        if($wxinfo) {
            ...
            switch ($loginRes['code']) {
                ...
                case GlobalCode::API_NO_ACCOUNT :
                    ...
                    break;
                case GlobalCode::API_REG_NO_FIN :
                    //尚未注册完成，跳转到注册流程
                ...
                case GlobalCode::API_FORBIDDEN_ACCOUNT :
                    ...
                    break;
                case GlobalCode::API_API_ERROR :
                    $this->dealReturn(false, '登录失败');
                    break;
            }
        } else {
            $this->failReturn('get user wxinfo faild', GlobalCode::API_ERROR, '获取微信接口调用失败，请重试');
        }
    }
            ...
            if ($user) {
                ...
                if(GlobalCode::SUCCESS == $cres) {
                    ...
                    if($this->setLogin($user)) {
                        ...
                        $res['code'] = GlobalCode::API_API_ERROR;
                    }
                } else {
                    $res['code'] = $cres;
                }
     ...
     */
    public function webCheckLogin()
    {
        $user = $this->getLoginer();
        if (!$user) {
            $this->Common->failReturn(GlobalCode::API_NO_LOGIN);
        }
    }
